...
public class MenuScreen implements Screen {
	...
	public void show() {
		...
		for (int i = 0; i < menuTexts.length; i++) {
			menuTexts[i].setPosition(getHalfX(banner) - menuTexts[i].getWidth() / 2, getTopY(banner) - 20 - i * separacion);
		}
		// the sprite was soooo big
		parchment = new Sprite(new Texture(Resources.PARCHMENT));
		parchment.setSize(400, 250);
		parchment.setPosition(Config.WIDTH/2-parchment.getWidth()/2,Config.HEIGHT/2 - parchment.getHeight()/2);
	...
	private void update(float delta) {
		...
		menuTexts[0].reset();
			...
			Render.app.setScreen(new MapScreen(scene.getIp(),Integer.parseInt(scene.getPort())));
